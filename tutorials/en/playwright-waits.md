...
- **Hard Wait**: A stop sign that forces _everyone_ to stop for 5 minutes, even at 3 AM with no other cars. Efficiency: 0%.
...
If the UI is _really_ slow (animation), wait for the data explicitly.
...
- If the app takes 6s, your test fails anyway.
  **The Fix**: Determine _what_ you are waiting for (text? visibility? url?) and assert that instead.
