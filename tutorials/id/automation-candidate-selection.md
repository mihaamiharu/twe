...
description: 'Pilih pemeriksaan yang bernilai, tentukan feedback layer yang tepat, dan rumuskan tujuan otomasi kecil yang bisa diulang.'
---

## Setelah lesson ini, kamu bisa

- menilai kandidat otomasi berdasarkan risiko, frekuensi, kendali atas kondisi, bukti yang bisa diamati, dan biaya perawatan;
- menjelaskan apakah UI browser, test layer yang lebih rendah, atau testing oleh manusia memberikan feedback paling berguna;
- memperkecil alur pengguna yang panjang menjadi test yang lebih mudah didiagnosis;
- mendokumentasikan tujuan otomasi sebelum menulis kode; dan
...
Nilai setiap kandidat melalui enam sudut pandang berikut. Ini bukan rumus atau sistem skor otomatis. Pertanyaannya dipakai supaya trade-off yang tadinya tersembunyi jadi kelihatan.

| Sudut pandang           | Pertanyaan                                                                     | Tanda peringatan                                                               |
| ----------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| Risiko dan nilai        | Kegagalan penting apa yang akan dideteksi?                                     | Nggak ada yang bisa menjelaskan dampak kegagalannya.                           |
| Frekuensi               | Seberapa sering tim membutuhkan feedback ini?                                  | Pemeriksaan kemungkinan besar cuma dijalankan sekali.                          |
| Kendali                 | Bisakah state dan test data dibuat lalu di-reset?                              | Test bergantung pada data bersama, nggak terduga, atau harus disiapkan manual. |
| Bukti                   | Bisakah outcome dibuktikan dengan jelas?                                       | “Kelihatannya benar” menjadi satu-satunya bukti.                               |
| Perubahan dan perawatan | Seberapa sering alur dan ekspektasinya berubah?                                | Fitur masih berupa eksperimen yang berubah hampir setiap hari.                 |
| Kebutuhan browser       | Apakah perilaku yang dilihat pengguna memang harus dibuktikan melalui browser? | Browser menambah biaya tanpa memberikan bukti tambahan.                        |

Kandidat nggak harus punya kondisi yang sempurna. Misalnya, sebuah skenario punya risiko tinggi tetapi datanya belum bisa dikendalikan. Kesimpulannya belum tentu “jangan pernah diotomasi.” Bisa jadi tim perlu memperbaiki **testability**, yaitu kemudahan aplikasi untuk disiapkan, diamati, dan diuji, sebelum membuat test tersebut.
...
| -------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Otomasi UI browser               | Bisakah pengguna menyelesaikan alur penting melalui halaman yang sebenarnya?                    | Pengguna mengirim form login valid lalu masuk ke area yang membutuhkan autentikasi. |
| API atau service                 | Apakah rule dan integrasi bekerja untuk banyak variasi data tanpa membuka halaman?              | Service menerima kredensial valid dan menolak kombinasi yang salah.                 |
| Unit atau component              | Apakah logika kecil atau bagian UI yang terisolasi bekerja dengan cepat?                        | Fungsi validasi menangani format input dan nilai batas.                             |
| Testing manual atau eksploratori | Apakah ada risiko baru, perilaku membingungkan, atau pengalaman yang sulit dinilai oleh script? | Mengeksplorasi apakah pesan error membantu pengguna memperbaiki masalah.            |
...
Fitur login biasanya membutuhkan gabungan beberapa layer. Mungkin ada banyak pemeriksaan cepat pada service, beberapa alur browser, security testing, dan sesi eksploratori. Nggak ada satu layer yang harus “menang.” Yang kita cari adalah cakupan gabungan yang berguna.
...
Kalau hasilnya sangat subjektif—misalnya “apakah desain baru terasa meyakinkan?”—testing manual dan eksploratori tetap dibutuhkan.
...
Catatan tujuan otomasi membantu QA, developer, dan stakeholder produk meninjau rencana test tanpa terjebak debat syntax.
...
Kondisi awal dan test data:
Aksi pengguna:
Outcome bisnis yang bisa diamati:
Risiko perawatan:
...
- Risiko produk apa yang sebenarnya ingin diuji?
...
Minta AI menunjukkan keputusan tersebut, bukan mengambilnya diam-diam. Prompt yang baik menjelaskan risiko, batasan, cara setup data, dan bukti yang dibutuhkan. Setelah itu, minta AI mencari asumsi yang hilang serta menawarkan alternatif.
...
3. Apa kondisi awal dan test data yang dibutuhkan?
4. Aksi dan outcome apa yang harus masuk ke dalam scope?
...
- **Keputusan:** buat alur UI yang kecil karena pemeriksaan ini berulang, penting, bisa dikendalikan, dan punya bukti yang jelas di browser.
...
- **Kondisi dan data:** gunakan keranjang yang bisa di-reset dan produk khusus testing. Jangan bergantung pada produk pertama yang kebetulan muncul.
- **Bukti:** periksa identitas produk, jumlah, dan subtotal yang dimaksud—bukan cuma memastikan halaman keranjang terbuka.
...
4. kondisi awal dan test data yang dibutuhkan; dan
5. bukti yang bisa diamati untuk menunjukkan hasilnya.
