...
test.describe('Expanded Application Scenarios', () => {
    ...
    test('Dashboard: should display core components and stats', async ({ page }) => {
        ...
        const section = dashboardPage.page.locator('section').filter({ has: dashboardPage.page.getByRole('heading', { name: /Featured Challenges/i }) });
        const challenges = section.locator('a.glass-card');
        await expect(challenges).toHaveCount(3);
    });

    test('Profile: should display user information and tabs', async ({ page }) => {
    ...
    test('Settings: should allow navigating back to profile', async ({ page }) => {
        await settingsPage.goto();
        console.log('Settings URL:', page.url());

        // Verify we are on settings page
        await expect(settingsPage.page.getByRole('heading', { level: 1, name: /Settings/i }).first()).toBeVisible();
