...
test.describe('Bug Reporting Flow', () => {
    ...
    test('should submit a bug report successfully', async ({ page }) => {
        ...
        // Try footer first
        const footerBugButton = page.locator('footer').getByRole('button', { name: /Report a Bug|Laporkan Bug/i });
        ...
        try {
            ...
            await page.locator('header').getByRole('button', { name: /avatar|user menu/i }).first().click();
            await page.getByRole('menuitem', { name: /Report Bug|Laporkan Bug/i }).click();
            await expect(page.getByRole('dialog')).toBeVisible({ timeout: 5000 });
        }

        // Fill the form
        await page.getByLabel(/Bug Title|Judul Bug/i).fill('E2E Test Bug Report');
        await page.getByLabel(/Steps to Reproduce|Langkah/i).fill('1. Open app\n2. Click button\n3. See error');
        await page.getByLabel(/Expected Behavior|Perilaku yang diharapkan/i).fill('It should work');
        await page.getByLabel(/Actual Behavior|Perilaku sebenarnya/i).fill('It failed');

        // Submit
        await page.getByRole('button', { name: /Submit Bug Report|Kirim Laporan Bug/i }).click();

        // Verify success toast/message
        await expect(page.getByText(/submitted successfully|berhasil dikirim/i)).toBeVisible();
    });
    ...
    test('admin should be able to see the bug report', async ({ page }) => {
        ...
        await expect(page.getByText('E2E Test Bug Report')).toBeVisible({ timeout: 10000 });
    });
});
