...
| `video`             | status, title, duration, focus, optional `embedUrl`     | Use `planned` until a video is ready. A ready video must use an HTTPS YouTube `/embed/` URL; arbitrary iframe HTML is not allowed.              |
