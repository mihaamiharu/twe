...
export function validateXPath(xpath: string): ValidationResult {
  ...
  try {
    ...
    return {
      isValid: false,
      error:
        error instanceof Error ? error.message : 'Invalid XPath expression',
    };
  }
}
...
export function testSelectorAgainstTarget(
  ...
  if (targetElementId) {
    ...
    if (targetElement) {
      const matchesTarget = userMatches.some(
        (match) =>
          match === targetElement || (match as Element).id === targetElementId,
      );
      ...
      if (userCount === 0) {
        return {
          ...
          feedback:
            'Your selector did not match any elements. Try a different approach.',
        };
      }
      ...
      return {
        ...
        expectedMatchCount: 1,
        feedback:
          'Perfect! Your selector uniquely identifies the target element.',
      };
    }
  }
  ...
  return {
    ...
    expectedMatchCount: expectedCount,
    feedback:
      'Your selector matches a different set of elements than expected.',
  };
}
...
export function generateSelectorHint(
  ...
  if (matchCount === 0) {
    ...
    if (selectorType === 'css') {
      hints.push(
        'Add more specificity with classes, IDs, or attribute selectors',
      );
