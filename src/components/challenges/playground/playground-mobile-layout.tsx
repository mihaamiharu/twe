...
export function PlaygroundMobileLayout({
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col bg-background">
      <Tabs
        value={
          activeTab === 'instructions' || activeTab === 'results'
            ? 'workspace'
            : activeTab
        }
      ...
      >
        ...
        <div className="px-4 pt-3 pb-2 flex items-center justify-between gap-3 shrink-0 border-b">
          <TabsList className="h-11 bg-muted/50 p-1 border border-border rounded-lg flex-1 grid grid-cols-2">
            <TabsTrigger value="workspace" className="text-xs gap-2">
              <Code2 className="h-3.5 w-3.5" />
              {t('challenges:playground.workspace')}
            </TabsTrigger>
            <TabsTrigger value="preview" className="text-xs gap-2">
              <Search className="h-3.5 w-3.5" />
              {t('challenges:playground.visual')}
            </TabsTrigger>
          </TabsList>
          ...
          <div className="flex items-center gap-1.5">
            <Sheet>
              <SheetTrigger asChild>
                <Button
                  ...
                  aria-label={t('challenges:playground.openInstructions')}
                >
              ...
              <SheetContent
                side="bottom"
                className="h-[70vh] rounded-t-2xl px-6 pb-6 pt-2 flex flex-col"
              >
            ...
            {challenge.files && Object.keys(challenge.files).length > 1 && (
              <Sheet
                open={isFileExplorerSheetOpen}
                onOpenChange={setIsFileExplorerSheetOpen}
              >
                <SheetTrigger asChild>
                  <Button
                    variant="outline"
                    size="icon"
                    className="size-11 rounded-lg"
                    aria-label={t('challenges:playground.openFiles')}
                  >
                ...
                <SheetContent
                  side="bottom"
                  className="h-[70vh] rounded-t-2xl p-0 overflow-hidden flex flex-col"
                >
              ...
              >
                <SheetTrigger asChild>
                  <Button
                    ...
                    aria-label={t('challenges:playground.openResults')}
                  >
                ...
                <SheetContent
                  side="bottom"
                  className="h-[60vh] rounded-t-2xl p-0 overflow-hidden flex flex-col"
                >
          ...
          >
            {hasHtml ? (
              ...
              <div className="flex-1 flex items-center justify-center p-8 text-center text-muted-foreground italic">
                {t('challenges:playground.noVisualPreview')}
              </div>
            )}
          </TabsContent>
        </div>
