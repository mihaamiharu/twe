...
export async function getUserStats(userId: string): Promise<UserStats> {
    ...
    // Calculate streak
    const { currentStreak, longestStreak } = calculateStreak(completedChallenges.map(c => c.completedAt).filter(Boolean) as Date[]);
 ...
 */
function calculateStreak(completionDates: Date[]): { currentStreak: number; longestStreak: number } {
    ...
    const isActiveStreak = firstDay.getTime() === today.getTime() ||
 ...
 */
export async function getEarnedAchievementIds(userId: string): Promise<Set<string>> {
...
export async function awardAchievements(
    ...
    const totalXpReward = dbAchievements.reduce((sum, a) => sum + (a.xpReward || 0), 0);
