 ...
 */
function parseFrontmatter(content: string): { meta: { title?: string; description?: string }; content: string } {
 ...
 */
export async function getTutorialContent(slug: string, locale: string): Promise<Tutorial | null> {
    try {
 ...
 */
export async function getTutorialList(locale: string): Promise<Omit<Tutorial, 'content'>[]> {
    const registry = await loadRegistry();
...
const TIER_FILES: ChallengeTier[] = ['basic', 'beginner', 'intermediate', 'expert'];
...
async function loadAllChallenges(): Promise<Map<string, ChallengeDefinition>> {
    ...
    for (const tier of TIER_FILES) {
        try {
            ...
            console.log(`[ContentService] Tier file ${tier}.json not found, skipping`);
        }
    }
 ...
 */
export async function getChallengeContent(slug: string, locale: string): Promise<Challenge | null> {
...
export async function getChallengeList(
    ...
    const challenges = await loadAllChallenges();
    let results: Omit<Challenge, 'testCases' | 'solution'>[] = [];
    ...
    for (const def of challenges.values()) {
        ...
        if (filters?.search) {
            ...
            const titleMatch = resolveLocale(def.title, locale).toLowerCase().includes(searchLower);
            const descMatch = resolveLocale(def.description, locale).toLowerCase().includes(searchLower);
