      ...
      if (challenge) {
        if (userProgress) {
          await db
            ...
            .where(
              and(
                eq(progress.id, userProgress.id),
                eq(progress.isCompleted, false),
              ),
            );
        } else {
          ...
          // A completion request may have created the row between
          // the read above and this insert. Update the now-known
          // unique row so the hint penalty remains authoritative.
          await db
            .update(progress)
            .set({
              usedHint: true,
