...
async function verifyParity() {
    ...
    try {
        ...
        const dbTutorials = await db.execute(sql`SELECT slug, title, description, content FROM tutorials`);
        ...
        for (const row of dbTutorials as any[]) {
            if (typeof row.title !== 'object' || !row.title.en) {
                console.error(`❌ Tutorial ${row.slug}: Title is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
            if (typeof row.content !== 'object' || !row.content.en) {
                console.error(`❌ Tutorial ${row.slug}: Content is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
        }
        ...
        // 2. Verify Challenges
        console.log('\n--- Challenges ---');
        const dbChallenges = await db.execute(sql`SELECT slug, title, instructions FROM challenges`);
        for (const row of dbChallenges as any[]) {
            if (typeof row.title !== 'object' || !row.title.en) {
                console.error(`❌ Challenge ${row.slug}: Title is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
            if (typeof row.instructions !== 'object' || !row.instructions.en) {
                console.error(`❌ Challenge ${row.slug}: Instructions is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
        }
        ...
        console.log('\n--- Achievements ---');
        const dbAchievements = await db.execute(sql`SELECT slug, name, description FROM achievements`);
        for (const row of dbAchievements as any[]) {
            if (typeof row.name !== 'object' || !row.name.en) {
                console.error(`❌ Achievement ${row.slug}: Name is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
            if (typeof row.description !== 'object' || !row.description.en) {
                console.error(`❌ Achievement ${row.slug}: Description is not a valid JSONB object with "en" key.`);
                hasError = true;
            }
        }
