...
const challengeTypeConfig: Record<string, { color: string; icon: ReactNode }> =
  {
    ...
    TYPESCRIPT: {
      color: 'bg-sky-500/15 text-sky-800 border-sky-500/25 dark:text-sky-300',
      icon: <FileCode2 className="size-3.5" />,
    },
    PLAYWRIGHT: {
      color:
        'bg-violet-500/15 text-violet-800 border-violet-500/25 dark:text-violet-300',
      icon: <FileCode2 className="size-3.5" />,
    },
    CSS_SELECTOR: {
      color:
        'bg-blue-500/15 text-blue-800 border-blue-500/25 dark:text-blue-300',
      icon: <Palette className="size-3.5" />,
    },
    XPATH_SELECTOR: {
      color:
        'bg-teal-500/15 text-teal-800 border-teal-500/25 dark:text-teal-300',
...
const isComingSoon = (challenge: Challenge) =>
  challenge.tags?.includes('coming-soon') ?? false;
const isBossChallenge = (challenge: Challenge) =>
  challenge.slug.includes('boss');
...
export function ChallengesPage() {
  ...
  const difficulty = (searchParams.difficulty ||
    'all') as ChallengeFilterDifficulty;
  const status = (searchParams.status ||
    (searchParams.hideCompleted
      ? 'available'
      : 'all')) as ChallengeFilterStatus;
  ...
  const initialChallengesResponse = routeApi.useLoaderData();
  ...
  const filteredChallenges = useMemo(
    () =>
      challenges.filter((challenge: Challenge) => {
        ...
        if (status === 'available')
          return !challenge.isCompleted && !isComingSoon(challenge);
        return true;
      }),
  ...
  const groupedChallenges = useMemo(() => {
    ...
    return Object.entries(groups).sort(([a], [b]) => {
      ...
      const tierDifference =
        TIER_ORDER.indexOf(getTierFromCategory(a)) -
        TIER_ORDER.indexOf(getTierFromCategory(b));
      return (
        tierDifference ||
  ...
  const trackCounts = useMemo(
    () =>
      ALL_TRACKS.reduce<Record<string, number>>((counts, track) => {
        counts[track.id] = challenges.filter((challenge: Challenge) =>
          track.match(challenge),
        ).length;
        return counts;
      }, {}),
    [challenges],
  ...
  const completedCount = challenges.filter(
    (challenge: Challenge) => challenge.isCompleted,
  ).length;
    ...
    <div className="flex h-full flex-col">
      <div className="mb-7 px-1">
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
          {t('board.sidebarDescription')}
        </p>
      </div>
      <nav
        className="flex-1 space-y-5 overflow-y-auto pr-1"
        aria-label={t('board.trackNavigation')}
      >
        {SIDEBAR_GROUPS.map((group) => (
          <section key={group.title}>
            {group.title !== 'Overview' && (
              <h2 className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">
      ...
      <div className="mt-6 border-t border-border pt-5">
        ...
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-label={t('page.overallProgress')}
        >
          <div
      ...
      <PageContainer className="max-w-[1600px]">
        <div className="grid gap-6 lg:grid-cols-[270px_minmax(0,1fr)]">
          <aside className="sticky top-20 hidden h-[calc(100vh-6rem)] lg:block">
            <PaperSurface className="h-full p-5">
              <SidebarContent />
            </PaperSurface>
          </aside>
          ...
          <main className="min-w-0">
              ...
              <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
                <SheetTrigger asChild>
                  ...
                  >
                    ...
                    <span className="sr-only">{t('board.openTracks')}</span>
                  </Button>
                </SheetTrigger>
            ...
            <section className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
              ...
              <div className="flex flex-wrap gap-2">
                <StatPill
                  ...
                  label={t('board.challengeCount')}
                />
                <StatPill
                  value={`${progress}%`}
                  label={t('page.overallProgress')}
                />
              </div>
            </section>
            ...
            >
              <span>
                {t('board.resultsCount', { count: filteredChallenges.length })}
              </span>
              {hasActiveFilters && groupedChallenges.length > 0 && (
                <Button
                  variant="link"
                    ...
                    <div className="mb-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {t('board.categoryLabel')}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                        className="ml-auto rounded-full text-xs font-medium"
                      >
                        {t('board.itemsCount', {
                        ...
                        <AnimatePresence mode="popLayout">
                          {categoryChallenges.map((challenge) => (
                            <ChallengeListCard
                              ...
                              isComingSoon={isComingSoon(challenge)}
                              isBoss={isBossChallenge(challenge)}
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="hover:bg-transparent">
                              ...
                              <TableHead>{t('labels.challenge')}</TableHead>
                              <TableHead>{t('filters.status')}</TableHead>
                              <TableHead>{t('labels.type')}</TableHead>
                              <TableHead>{t('labels.difficulty')}</TableHead>
