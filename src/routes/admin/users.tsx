...
function UserManager() {
    ...
    const toggleBanMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            const status = variables.showOnLeaderboard ? 'Visible' : 'Hidden (Shadow Banned)';
        ...
        <div className="container mx-auto p-6 space-y-8 animate-fade-in">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage user visibility and access.</p>
                </div>
            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-right">Leaderboard Visibility</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredUsers.length === 0 ? (
                                ...
                                filteredUsers.map((user: any) => (
                                    <UserRow
                                        key={user.id}
                                        user={user}
                                        onToggle={(checked) => toggleBanMutation.mutate({ id: user.id, showOnLeaderboard: checked })}
                                        isPending={toggleBanMutation.isPending && toggleBanMutation.variables?.id === user.id}
                                    />
                                ))
                            )}
                        </TableBody>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 flex gap-3 text-sm text-yellow-700 dark:text-yellow-400">
                ...
                <div>
                    ...
                    <p className="mt-1 opacity-90">
                        Disabling "Leaderboard Visibility" will hide the user from all public leaderboards and rankings.
                        They can still log in and solve challenges, but their activity won't be visible to others.
                        This is effective for spam accounts or bad actors.
                    </p>
                </div>
            </div>
        </div>
...
function UserRow({ user, onToggle, isPending }: { user: any, onToggle: (c: boolean) => void, isPending: boolean }) {
    ...
    return (
        <TableRow>
            ...
            <TableCell>
                <Badge variant={isAdmin ? 'default' : 'secondary'} className={isAdmin ? 'bg-purple-500/15 text-purple-600 hover:bg-purple-500/25' : ''}>
                    {user.role}
                </Badge>
            </TableCell>
            <TableCell>
            ...
            <TableCell className="text-right">
                <div className="flex items-center justify-end gap-2">
                    <span className={`text-xs font-medium ${user.showOnLeaderboard ? 'text-green-600' : 'text-red-500'}`}>
