...
The expected smoke-test result is one passing test against the approved target. If the target is unavailable, save the original output and classify the issue before changing the test.
...
If Linux browser dependencies are missing, inspect the packages before using `npx playwright install --with-deps`. Do not use `sudo npm install`.
